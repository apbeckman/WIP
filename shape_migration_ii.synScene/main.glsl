...
float map(vec3 p){
    ...
    // pR(bF.xz, bF.y * (0.25+.15*cos(smoothTimeC*0.0712)));
    // pR(bF.yz, bF.x * -(0.35+_nsin(smoothTimeC*0.0911)*0.15));
    bF1 = bF;
    // bF1.x += 2.0;
    // bF.z -= 4.0;
    s.xz = abs(s.xz);
    ...
    // float cube1 = boxFrameSDF(bF1, bfSize.xyz, bfSize.w);
    // result = fOpUnionRound(cube, cube1, .2);
...
vec4 renderMainImage() {
	...
	vec3 lookAt = camPos + vec3(cos(TIME*0.6)*0.0125, sin(TIME*0.4)*0.0125, 0.5);  // "Look At" position.
